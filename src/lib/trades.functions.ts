...
export const getPublishedJournalMetrics = createServerFn({ method: "GET" }).handler(
  async (): Promise<JournalMetrics> => {
    ...
    try {
      ...
      // No placeholder trades: an empty journal reports zeroes rather than invented performance.
      return computeMetrics(trades);
    } catch (error) {
      console.error("[trades:metrics]", error);
      return computeMetrics([]);
    }
  },
);
...
export const listPublishedTrades = createServerFn({ method: "GET" }).handler(
  async (): Promise<TradeRecord[]> => {
    ...
    try {
      ...
      // No placeholder trades: an empty journal renders its empty state on the site.
      return trades;
    } catch (error) {
      console.error("[trades:published]", error);
      return [];
    }
  },
);
