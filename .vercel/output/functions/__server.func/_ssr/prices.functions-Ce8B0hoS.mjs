...
import { p as tickerItems } from "./site-data-C6vM7IrX.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/prices.functions-Ce8B0hoS.js
var cache = null;
var chartCache = null;
var TTL_MS = 45e3;
...
async function fetchTwelveDataQuotes(symbols) {
	...
	const mapped = symbols.map((symbol) => ({
		symbol,
		apiSymbol: TWELVE_DATA_SYMBOLS[symbol]
	})).filter((entry) => Boolean(entry.apiSymbol));
	if (!mapped.length) return {};
	const apiSymbols = [...new Set(mapped.map((entry) => entry.apiSymbol))].join(",");
	const res = await fetch(`https://api.twelvedata.com/quote?symbol=${encodeURIComponent(apiSymbols)}&apikey=${encodeURIComponent(apiKey)}`, { headers: { Accept: "application/json" } });
	if (!res.ok) throw new Error(`Twelve Data ${res.status}`);
	const json = await res.json();
	const quoteByApiSymbol = mapped.length === 1 && !json[mapped[0].apiSymbol] ? { [mapped[0].apiSymbol]: json } : json;
	const out = {};
...
async function fetchYahooQuotes(symbols) {
	const entries = symbols.map((symbol) => {
		...
		return source?.provider === "yahoo" ? {
			symbol,
			yahooSymbol: source.symbol
		} : null;
	}).filter((entry) => Boolean(entry));
	if (!entries.length) return {};
	const out = {};
	await Promise.all(entries.map(async ({ symbol, yahooSymbol }) => {
		const res = await fetch(`https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(yahooSymbol)}?range=1d&interval=1d`, { headers: {
...
async function fetchYahooQuoteBatch(symbols) {
	...
	const res = await fetch(`https://query1.finance.yahoo.com/v7/finance/quote?symbols=${encodeURIComponent(yahooSymbols)}`, { headers: {
		Accept: "application/json",
		"User-Agent": "Mozilla/5.0 (compatible; MarketEdgeAtlas/1.0)"
	} });
	if (!res.ok) return {};
...
var getLivePrices_createServerFn_handler = createServerRpc({
	id: "9dd1eb1e944d189bb9261befb0207f2caa11259d8cae85a28c66f0313373080a",
	name: "getLivePrices",
	filename: "src/lib/prices.functions.ts"
}, (opts) => getLivePrices.__executeServer(opts));
var getLivePrices = createServerFn({ method: "GET" }).validator((input) => livePricesInput.parse(input)).handler(getLivePrices_createServerFn_handler, async ({ data }) => {
	...
	if (cache && Date.now() - cache.at < TTL_MS && cache.key === key) return {
		quotes: cache.quotes,
		cached: true
	};
	try {
		const [twelveData, crypto, yahoo] = await Promise.all([
			fetchTwelveDataQuotes(symbols).catch(() => ({})),
			fetchCrypto(symbols).catch(() => ({})),
			fetchYahooQuoteBatch(symbols).then(async (batch) => {
				const missing = symbols.filter((symbol) => resolvePairSource(symbol)?.provider === "yahoo" && !batch[symbol]);
...
async function fetchBtcChart() {
	const [chartRes, ohlcRes, spotRes] = await Promise.all([
		fetch("https://api.coingecko.com/api/v3/coins/bitcoin/market_chart?vs_currency=usd&days=7", { headers: { Accept: "application/json" } }),
		fetch("https://api.coingecko.com/api/v3/coins/bitcoin/ohlc?vs_currency=usd&days=7", { headers: { Accept: "application/json" } }),
	...
	const candles = ohlcJson.map((row) => [
		row[1],
		row[2],
		row[3],
		row[4]
...
var getBtcMarketChart_createServerFn_handler = createServerRpc({
	id: "d5fb861f3dde8f622309aff684be437412f1ea464647c11a018605ccab9e1128",
	name: "getBtcMarketChart",
	filename: "src/lib/prices.functions.ts"
}, (opts) => getBtcMarketChart.__executeServer(opts));
var getBtcMarketChart = createServerFn({ method: "GET" }).handler(getBtcMarketChart_createServerFn_handler, async () => {
	if (chartCache && Date.now() - chartCache.at < TTL_MS) return chartCache.data;
	try {
...
//#endregion
export { getBtcMarketChart_createServerFn_handler, getLivePrices_createServerFn_handler };
